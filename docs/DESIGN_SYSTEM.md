...
- Secondary pages keep the theme's content width (`--max-content-width`, from `_config.yml` → `max_width`) for **both** the navbar and the content shell, so their visible edges line up. The homepage deviates by its documented 3rem optical compensation, and the `/portfolio/` dashboard declares its own 80rem shell (`--hao-portfolio-shell-max`) applied to the navbar, content and footer together, because the chart grid needs more canvas than a reading page. A shell is always one value for every surface on that page — widening only the content is what made the portfolio edges miss the navbar by 180px per side. `test/design` fails if a page's content edges stop matching its navbar, and if the portfolio shell stops being 80rem.
...
- Consent has to be withdrawable, not just given. When `enable_cookie_consent` is on, the footer carries a **Cookie settings** control that reopens the dialog (`assets/js/cookie-settings.js`), and `_pages/privacy.md` points at it. `test/design` clicks the control and asserts the preferences dialog opens, so the withdrawal path cannot rot.
- Repeated third-party embeds are deferred, not duplicated. The portfolio page ships each TradingView widget once per colour theme and once per analysis symbol; `assets/js/tradingview-deferred.js` promotes only the visible variant and re-checks on theme or tab changes. Author new embeds as `<script type="text/tradingview" data-src="...">` so the JSON stays in the source and never executes on its own.
